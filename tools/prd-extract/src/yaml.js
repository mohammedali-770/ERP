...
export function emitList(items, indent) {
    if (items.length === 0)
        return ' []';
    ...
    for (let lineNo = 0; lineNo < lines.length; lineNo++) {
        ...
        if (listItem) {
            if (!currentKey || !currentField)
                throw new Error(`${where}: list item outside a field`);
            const target = out[currentKey][currentField];
            if (!Array.isArray(target))
                throw new Error(`${where}: field ${currentField} is not a list`);
        ...
        if (field) {
            if (!currentKey)
                throw new Error(`${where}: field outside an entry`);
